...
- **Parakeet can drop a language it did not pick.** It detects the language once and on the deployment host produced no tokens at all for an English voice inside a Russian recording. A mixed-language conversation may lose its minority language under this backend.
