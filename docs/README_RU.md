...
Язык можно задать кодом (`ru`) или английским названием (`russian`); всё, чего модель не знает, отклоняется с `400`, а не падает посреди расшифровки. `GET /api/models` перечисляет то, что она знает.
